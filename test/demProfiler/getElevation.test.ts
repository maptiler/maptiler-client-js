...
const pngTile = async () => await sharp("test/fixtures/12-775-1538.png").raw().toBuffer();
const webpTile = async () => await sharp("test/fixtures/13-1548-3076.webp")
  .raw()
  .toBuffer();
...
describe("getElevation", () => {
  it("png: returns the elevation of a point", async () => {
    ...
    const tileImage = {
      channels: 3 as const,
      image: new Uint8ClampedArray(await pngTile()),
    };
  ...
  it("webp: returns the elevation of a point", async () => {
    ...
    const tileImage = {
      channels: 3 as const,
      image: new Uint8ClampedArray(await webpTile()),
    };
    const elevation = getElevation(coord, tile, tileSize, tileImage);
